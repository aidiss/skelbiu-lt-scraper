...
class SkelbiuPipeline(object):
    def process_item(self, i, spider):
        ...
        listing['price'] = i['left-content'].css('.price::text').extract_first().strip()
        # if item['price']:
        #     item['price'] = item['price'][:-1].split(':')[-1].replace(' ', '').strip()
        # item['description'] = '\n'.join([x for x in item['description'] if x.strip()])

        listing['description'] = i['left-content'].css('.item-description').extract_first().strip()

        listing['phone'] = i['right-content'].css('.phone-button>.primary::text').extract_first().strip()
